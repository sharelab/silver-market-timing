...
The silver commodity (spot symbol: `^XAGUSD`) can be thought of in terms of a zero-coupon paying bond with an infinite time to maturity. Assuming cashing-out within finite time, we are interested in the asset's spot price at the end of the arbitrary holding period, $T$. 

In most simplistic terms, any physical asset appreciates/depreciates in nominal value with inflation/deflation. It follows that the future silver spot price, $P_T$, is a function of the spot price at the beginning of the investment period, $P_0$, and USD inflation, $i$. In addition, there might be some $n$ other dependencies, which might be observable or not, and where it is not clear whether they have a positive or negative impact on $P_T$. These are included in the vector $X_n$, $X_n = [x_1, x_2,..., x_n]$.
